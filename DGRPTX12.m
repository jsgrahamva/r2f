...
X11 S DFN=DA D SV^DGLOCK
 Q
 ;
12 D:$D(DG)>9 F^DIE17,DE S DQ=12,DW=".3;1",DV="SXa",DU="",DLB="SERVICE CONNECTED?",DIFLD=.301
 S DE(DW)="C12^DGRPTX12"
 S DU="Y:YES;N:NO;"
 G RE
...
19 D:$D(DG)>9 F^DIE17,DE S DQ=19,DW=".36;1",DV="*P8'Xa",DU="",DLB="PRIMARY ELIGIBILITY CODE",DIFLD=.361
 S DE(DW)="C19^DGRPTX12",DE(DW,"INDEX")=1
 S DU="DIC(8,"
 G RE
...
C19S S X="" G:DG(DQ)=X C19F1 K DB
 D ^DGRPTX15
C19F1 N X,X1,X2 S DIXR=799 D C19X1(U) K X2 M X2=X D C19X1("O") K X1 M X1=X
 D
...
20 D:$D(DG)>9 F^DIE17,DE S DQ=20,D=0 K DE(1) ;361
 S DIFLD=361,DGO="^DGRPTX16",DC="3^2.0361IP^E^",DV="2.0361M*P8'X",DW="0;1",DOW="ELIGIBILITY",DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DIC(8,"
...
R20 D DE
 ...
 ;
21 D:$D(DG)>9 F^DIE17 G ^DGRPTX17
