...
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,21) S:%]"" DE(9)=%
 I $D(^("M")) S %Z=^("M") S %=$P(%Z,U,11) S:%]"" DE(6)=% S %=$P(%Z,U,14) S:%]"" DE(1)=%
 I $D(^("M1")) S %Z=^("M1") S %=$P(%Z,U,2) S:%]"" DE(13)=%,DE(24)=% S %=$P(%Z,U,10) S:%]"" DE(14)=%,DE(25)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="M;14",DV="FOX",DU="",DLB="TERTIARY INSURANCE POLICY",DIFLD=114
...
C1F1 S DIEZRXR(399,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 ...
 Q
X1 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>20!($L(X)<1) X D:$D(X) DD^IBCNS2(X,DA,3)
 I $D(X),X'?.ANP K X
 Q
 ;
2 S DQ=3 ;@998
...
X5 D INST^IBCU
 Q
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="M;11",DV="RP4",DU="",DLB="RESPONSIBLE INSTITUTION",DIFLD=111
 S DE(DW)="C6^IBXSC33"
 S DU="DIC(4,"
 G RE
...
X7 K DIC("DR"),DLAYGO
 Q
8 S DQ=9 ;@31
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW="0;21",DV="*S",DU="",DLB="CURRENT BILL PAYER SEQUENCE",DIFLD=.21
 S DE(DW)="C9^IBXSC33"
...
X10 S:IBDR20'["32" Y="@33"
 Q
11 S DQ=12 ;@3211
12 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=12 D X12 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X12 I '$$SUPPPT^IBCEP7B(DA,1) S Y="@3212"
 Q
13 D:$D(DG)>9 F^DIE17,DE S DQ=13,DW="M1;2",DV="FX",DU="",DLB="PRIMARY PROVIDER #",DIFLD=122
 S DE(DW)="C13^IBXSC33"
 S Y="@"
...
X13 K:$L(X)>13!($L(X)<3)!($TR(X," ")="") X
 ...
 Q
 ;
14 D:$D(DG)>9 F^DIE17,DE S DQ=14,DW="M1;10",DV="*P355.97'",DU="",DLB="PRIMARY ID QUALIFIER",DIFLD=128
 S DU="IBE(355.97,"
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
...
X15 W !,"No Billing Prov Secondary IDs will be sent for Primary Ins/Elect Plan Type."
 Q
...
X23 S IBPSDAT=$$GETID^IBCEP7B(DA,1),IBPSID=$P(IBPSDAT,U),IBPSQUAL=$P(IBPSDAT,U,2) I IBPSID="" S Y="@321"
 Q
24 S DW="M1;2",DV="FX",DU="",DLB="PRIMARY PROVIDER #",DIFLD=122
 S DE(DW)="C24^IBXSC33"
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
...
X24 K:$L(X)>13!($L(X)<3)!($TR(X," ")="") X
 ...
 Q
 ;
25 D:$D(DG)>9 F^DIE17,DE S DQ=25,DW="M1;10",DV="*P355.97'",DU="",DLB="PRIMARY ID QUALIFIER",DIFLD=128
 S DU="IBE(355.97,"
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
...
X28 S IBPSIDO=$P($G(^DGCR(399,DA,"M1")),U,2),IBPSQO=$P($G(^DGCR(399,DA,"M1")),U,10)
 Q
29 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=29 G A
30 D:$D(DG)>9 F^DIE17 G ^IBXSC34
