...
CPUTIME()	;Returns two comma-delimited pieces, "system" CPU time and "user"
	...
	;Q $SYSTEM.Process.GetCPUTime() gpl
	;GT.M
	Q $ZGETJPI("","CPUTIM")*10 ; gpl
	;
	;=============================
ETIMEMS(START,END)	;Calculate and return the elapsed time in milliseconds.
	;START and STOP times are microseconds from $ZUT
	Q $J(END-START/1E3,0,0)_" milliseconds"
	;
	;=============================
...
XOR(X,Y)	;Bitwise logical XOR, 32 bits. IA #6157
	...
	;=============================
TESTS	;
	N END,HASH,HASHLEN,IND,JND,LEN,LINE,MSG,NBLOCKS,REFHASH,REPS,START,STR,TMP1,TMP2
	W !,"Starting the tests."
	F IND=1:1 S LINE=$P($T(TESTVEC+IND),";;",2) Q:LINE=-1  D
	. I LINE["msg" D
	.. S STR=$P(LINE,":",2),REPS=$P(LINE,":",3)
	.. ;S MSG=$S(STR="": "the null string",1:STR) ; gpl before
	.. S MSG=$S(STR="":"the null string",1:STR) ; gpl after
	...
	.. I LEN<32767 D
	... S TMP1=+$P($ZV,"V",2),TMP2=$H,START=$S(TMP1>6.2!(TMP1=6.2&(+$P(TMP1,"-",2)>1)):$ZUT,1:$P(TMP2,",",1)*88400+$P(TMP2,",",2)*1E6)
	... S HASH=$$SHAN^XLFSHAN(HASHLEN,STR)
	... S TMP1=+$P($ZV,"V",2),TMP2=$H,END=$S(TMP1>6.2!(TMP1=6.2&(+$P(TMP1,"-",2)>1)):$ZUT,1:$P(TMP2,",",1)*88400+$P(TMP2,",",2)*1E6)
