...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(.11)) S %Z=^(.11) S %=$P(%Z,U,1) S:%]"" DE(2)=% S %=$P(%Z,U,2) S:%]"" DE(3)=% S %=$P(%Z,U,3) S:%]"" DE(4)=% S %=$P(%Z,U,4) S:%]"" DE(5)=% S %=$P(%Z,U,5) S:%]"" DE(6)=% S %=$P(%Z,U,7) S:%]"" DE(8)=% S %=$P(%Z,U,12) S:%]"" DE(7)=%
...
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW=".11;1",DV="FaR",DU="",DLB="STREET ADDRESS [LINE 1]",DIFLD=.111
 S DE(DW)="C2^DVBAXA7",DE(DW,"INDEX")=1
 G RE
...
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".11;2",DV="Fa",DU="",DLB="STREET ADDRESS [LINE 2]",DIFLD=.112
 S DE(DW)="C3^DVBAXA7",DE(DW,"INDEX")=1
 G RE
...
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".11;3",DV="Fa",DU="",DLB="STREET ADDRESS [LINE 3]",DIFLD=.113
 S DE(DW)="C4^DVBAXA7",DE(DW,"INDEX")=1
 G RE
...
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW=".11;4",DV="FaR",DU="",DLB="CITY",DIFLD=.114
 S DE(DW)="C5^DVBAXA7",DE(DW,"INDEX")=1
 G RE
...
X5 K:$L(X)>15!($L(X)<2) X
 ...
 Q
 ;
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW=".11;5",DV="*P5'aR",DU="",DLB="STATE",DIFLD=.115
 S DE(DW)="C6^DVBAXA7",DE(DW,"INDEX")=1
 S DU="DIC(5,"
 G RE
...
C6F1 N X,X1,X2 S DIXR=235 D C6X1(U) K X2 M X2=X D C6X1("O") K X1 M X1=X
 D
...
C6X1(DION) K X
 ...
 Q
C6F2 Q
X6 S DIC("S")="I $P(^DIC(5,Y,0),U,6)=1" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW=".11;12",DV="FXOaR",DU="",DLB="ZIP+4",DIFLD=.1112
 S DQ(7,2)="S Y(0)=Y D ZIPOUT^VAFADDR"
 S DE(DW)="C7^DVBAXA7",DE(DW,"INDEX")=1
 G RE
...
C7S S X="" G:DG(DQ)=X C7F1 K DB
 D ^DVBAXA8
C7F1 N X,X1,X2 S DIXR=185 D C7X1(U) K X2 M X2=X D C7X1("O") K X1 M X1=X
 D
...
C7X1(DION) K X
 ...
 S X=$G(X(1))
 Q
C7F2 S DIXR=231 D C7X2(U) K X2 M X2=X D C7X2("O") K X1 M X1=X
 D
...
C7X2(DION) K X
 ...
 Q
C7F3 Q
X7 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>20!($L(X)<5) X I $D(X) D ZIPIN^VAFADDR
 I $D(X),X'?.ANP K X
 Q
 ;
8 D:$D(DG)>9 F^DIE17,DE S DQ=8,DW=".11;7",DV="NJ3,0XOaR",DU="",DLB="COUNTY",DIFLD=.117
 S DQ(8,2)="S Y(0)=Y Q:Y']""""  S Z0=$S($D(^DPT(D0,.11)):+$P(^(.11),""^"",5),1:"""") Q:'Z0  S Y=$P($S($D(^DIC(5,Z0,1,Y,0)):^(0),1:""""),""^"",3)"
 S DE(DW)="C8^DVBAXA7"
 G RE
...
C8S S X="" G:DG(DQ)=X C8F1 K DB
 D ^DVBAXA10
C8F1 Q
X8 S Z0=$S($D(^DPT(D0,.11)):+$P(^(.11),"^",5),1:0) K:'Z0 X Q:'Z0!'$D(^DIC(5,Z0,1,0))  S DIC="^DIC(5,Z0,1,",DIC(0)="QEM" D ^DIC S X=+Y K:Y'>0 X K Z0,DIC
 Q
 ;
9 D:$D(DG)>9 F^DIE17 G ^DVBAXA11
