HLOSRVR	;ALB/CJM - Server for receiving messages - 10/4/94 1pm ;4:42 PM  18 May 2015
	;;1.6;HEALTH LEVEL SEVEN;**126,130,131,134,137,138,139,143,147,157,158,WVEHR,LOCAL**;Oct 13, 1995;Build 1
...
LINUX(LINKNAME)	;Listener for Linux systems running under Xinetd.
	...
	Q:$$CHKSTOP^HLOPROC
	;
GTMLNX	;Begin WorldVistA change
	; OS service may be defined using xinetd or equivalent.
	; Service script should call here, passing the registered service name.
	;was;D $ZU(68,15,1) ;need error on disconnect
	;was;D SERVER(LINKNAME,$PRINCIPAL)
	N PROC,LINKNAME,NODE
	S PROC=$ZCMDLINE Q:'$L(PROC)  ;HLO PROCESS REGISTRY name passed-in from OS script
	S PROC=$O(^HLD(779.3,"B",PROC,0)) Q:'(PROC>0)  ;IEN
	I PROC S LINKNAME=$P($G(^HLD(779.3,PROC,0)),"^",14) ;HL LOGICAL LINK name
	S NODE=$G(^HLD(779.1,1,0)) ;HLO SYSTEM PARAMETERS 0-node
	I '$L(LINKNAME) N % S %=$P(NODE,"^",10) I %>0 S LINKNAME=$P($G(^HLCS(870,%,0)),"^")
	Q:'$L(LINKNAME)  ;HL LOGICAL LINK from either HLO PROCESS REGISTRY =or= HLO SYSTEM PARAMETERS
	D SERVER(LINKNAME,"GTM$LNX") ;LOGICAL detected in OPEN^HLOTCP
...
INQUE(MSGIEN,PARMS)	;
	...
	;
	;Begin WorldVistA change
	;SIS/LM - Need the equivalent of IMMEDIATE for GT.M testing
	;was;I ('$G(MSGIEN))!(INQUE>20) S MSGIEN=0 D
	I ('$G(MSGIEN))!(INQUE>20)!($G(HLCSTATE("SYSTEM","OS"))="GT.M") S MSGIEN=0 D
	.;End WorldVistA change
