MAG7RS ;WOIFO/PMK,MLH - copy radiology message from HLSDATA to ^MAGDHL7 - add segment data ; 30 Jun 2004  4:32 PM
 ;;3.0;IMAGING;**11,40,30**;16-September-2004;Build 2
 ;; +---------------------------------------------------------------+
...
PIDADD ; SUBROUTINE - called by ADDDTA^MAGDHL7
 ...
 N ENGNM,ENGNMOUT ; ---------------- English Name components
 N ENGNMWRK
 ;
 ; ====================================================
 ...
 ;begin changes for English Name support for Jordan - saf ISI 9/25/11
 I $G(^DD(2,400000011,0))'="" D  
 . I VADM(1)'="" S VADM(1)=$$ENGPAT^MAG7RS(DFN,VADM(1))  ; saf ISI - get English name if exists
 . S ENGNM=$TR(VADM(1),",","~")  ;,ENGNM=$TR(ENGNM," ","~")
 . S ENGNMWRK="",ENGNMWRK(1)="MSH^~|\&^^^^^^^^^^^^^^^^"
 . S ENGNMWRK(2)="PID^^^^^"_ENGNM_"^^^^^^^^^^^^^^"
 . I $$PARSE^MAG7UP("ENGNMWRK","ENGNMOUT")=0 D  
 . . K MAG7WRK(2,5)
 . . M MAG7WRK(2,5)=ENGNMOUT(2,5)
 . . Q
 . Q
 I '$G(VAERR),VADM(3)]"" D
...
PV1RAD ; SUBROUTINE - called by ADDVSDG
 ...
 Q
 ;
ENGPAT(DFN,PATNM) ;Use English Patient Name if it exists
 N ENGPATNM
 I $$GET1^DID(2,400000011,"","LABEL")="" Q PATNM  ;ENGLISH NAME field doesn't exist - return name that was passed in 
 S ENGPATNM=$$GET1^DIQ(2,DFN_",",400000011)
 I ENGPATNM'="" Q ENGPATNM
 Q PATNM   ;English name is null... return name that was passed in.
