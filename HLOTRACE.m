HLOTRACE	;;4:42 AM  22 Mar 2014;03/07/2011
	;;1.6;HEALTH LEVEL SEVEN;**146,147,153,WVEHR,LOCAL** ;Oct 13, 1995;Build 3
...
START	;
	...
	;Begin WorldVistA mod 2-20-2012 DJW
	;Was;I $P($$VERSION^%ZOSV(1),"/",1)'["Cache" D  Q
	;Was;.W ! !,"   Sorry, this tool can only be used under Cache",!!
	I "^Cache^GT.M^"'[(U_$P($$VERSION^%ZOSV(1),"/",1)_U) D  Q
	.W !!,"   Sorry, this tool can only be used under Cache and GT.M",!!
	;End WorldVistA mod 2-20-2012 DJW
	...
	Q:QUE=""
	;Begin WorldVistA mod 2-20-2012 DJW
	;was ;ZB /INTERRUPT:NORMAL ;disable CTRL-C breaks
	I $P($$VERSION^%ZOSV(1),"/",1)["Cache" ZB /INTERRUPT:NORMAL Q
	I $P($$VERSION^%ZOSV(1),"/",1)["GT.M"  U $P:(CENABLE) Q
	;End WorldVistA mod 2-20-2012 DJW
...
GTMBREAKS	;
	...
	;
	;set break in $$STOPPED^HLOQUE to circumvent shutdown of the queue
	ZB ZB0^HLOQUE:"S RET=0"
	;set break in $$IFSHUT^HLOTLNK to circumvent shutdown of the link
	ZB ZB0^HLOTLNK:"S RET=0"
	;set break at ZB0 in client (start of DOWORK)
	ZB ZB0^HLOCLNT:"D WRITE^HLOTRACE(""Launching the client process..."")"
	;set break at ZB1 in client ($$CONNECT)
	ZB ZB1^HLOCLNT1:"D WRITE^HLOTRACE(""Trying to connect..."")"
	;set break at ZB2 in client (end of $$CONNECT)
	ZB ZB2^HLOCLNT1:"D ZB2^HLOTRACE"
	;set break at ZB3 in client (ERROR TRAP)
	ZB ZB3^HLOCLNT:"D ZB3^HLOTRACE"
	;set break at ZB4 in client (FOR loop on the outgoing queue)
	ZB ZB4^HLOCLNT:"D ZB4^HLOTRACE"
	;set break at ZB5 in client (end of DOWORK, with just cleanup left)
	ZB ZB5^HLOCLNT:"D WRITE3^HLOTRACE(""Cleaning up...."")"
	;set break at ZB6 in client (start of $$TRANSMIT^HLOCLNT1)
	ZB ZB6^HLOCLNT1:"D WRITE^HLOTRACE(""Beginning to transmit message...."")"
	;set break at ZB7 in client (end of $$TRANSMIT^HLOCLNT1)
	ZB ZB7^HLOCLNT1:"D WRITE^HLOTRACE(""Message transmitted!"")"
	;set break at ZB8 in client (start of $$READACK^HLOCLNT1)
	ZB ZB8^HLOCLNT1:"D WRITE^HLOTRACE(""Beginning to read commit acknowledgment...."")"
	;set break at ZB9 in client (end of $$READACK^HLOCLNT1)
	;ZB ZB9^HLOCLNT1:"D WRITE^HLOTRACE(""Commit acknowledgment received!"")"
	ZB ZB9^HLOCLNT1:"D ZB9^HLOTRACE"
	;set break ZB10 in the client(start of $$READHDR^HLOT)
	ZB ZB10^HLOT:"D ZB10^HLOTRACE"
	;set break ZB11 in the client(end of $$READHDR^HLOT)
	ZB ZB11^HLOT:"D ZB11^HLOTRACE"
	;set break ZB12 in the client(start of $$READSEG^HLOT)
	ZB ZB12^HLOT:"D ZB12^HLOTRACE"
	;set break ZB13 in the client(end of $$READSEG^HLOT)
	ZB ZB13^HLOT:"D ZB13^HLOTRACE"
	;set break ZB14 in the client(start of $$WRITESEG^HLOT)
	ZB ZB14^HLOT:"D ZB14^HLOTRACE"
	;set break ZB15 in the client(end of $$WRITESEG^HLOT)
	ZB ZB15^HLOT:"D ZB15^HLOTRACE"
	;set break ZB16 in the client(start of $$WRITEHDR^HLOT)
	ZB ZB16^HLOT:"D ZB16^HLOTRACE"
	;set break ZB17 in the client(end of $$WRITEHDR^HLOT)
	ZB ZB17^HLOT:"D ZB17^HLOTRACE"
	;set break ZB18 in the client(start of $$ENDMSG^HLOT)
	ZB ZB18^HLOT:"D ZB18^HLOTRACE"
	;set break ZB19 in the client(end of $$ENDMSG^HLOT)
	ZB ZB19^HLOT:"D ZB19^HLOTRACE"
	ZB ZB20^HLOCLNT:"D ZB20^HLOTRACE"
	ZB ZB21^HLOCLNT:"D ZB21^HLOTRACE"
	ZB ZB22^HLOCLNT:"D ZB22^HLOTRACE"
	ZB ZB23^HLOCLNT:"D ZB23^HLOTRACE"
	ZB ZB24^HLOCLNT1:"D ZB24^HLOTRACE"
	ZB ZB25^HLOCLNT:"D WRITE^HLOTRACE(""Calling DEQUE..."")"
	Q
...
ZB3	;
	N CON,MSG
	;Begin WorldVistA Change DJW
	;was S CON=($ZA\8192#2)
	I $P($$VERSION^%ZOSV(1),"/",1)["Cache" S CON=($ZA\8192#2)
	;End WorldVistA Change DJW 3/2014
	S MSG="Error encountered, $ECODE="_$ECODE
	D WRITE^HLOTRACE(MSG)
	;Begin WorldVistA Change
	;was S MSG=$S(CON:"           TCP connection still active",1:"          TCP connection was dropped")
	I $P($$VERSION^%ZOSV(1),"/",1)["Cache"
