MAGBRTUT ;WOIFO/EdM,ISI/BT - Routing Utilities ; 1/14/2016 06:15 PM
 ;;3.0;IMAGING;**9,11,30,51,50,85,54,local**;03-July-2009;Build 1;Build 1424
...
SEND(IMAGE,LOC,PRI,MECH,FROM,ID) ; Enter image into Routing Queue
 ...
 . ;** ISI/BT local mod use user's division for the location where the routing request was made
 ...
 . . Q
 . ;
 . L +^MAGQUEUE(2006.035,0):1999999 ; Background job MUST wait
 ...
 ;
 ;** ISI/BT local mod
DIV() ;Get User's Division
 N DIV S DIV=$O(^VA(200,DUZ,2,"AX1",1,0)) ;default
 S:'DIV DIV=$O(^VA(200,DUZ,"B",0)) 
 Q DIV
 ;** end local mod
 ;
...
EVALPU(OUT,UNTIL) ; RPC = MAG DICOM ROUTE EVAL PURGE
 ...
 L +^MAGQUEUE(2006.03,0):1999999 ; Background process must wait
...
REMOVE(OUT,OLD,KEY) ; RPC = MAG DICOM XMIT REMOVE GATEWAY
 ...
 I KEY'=5,KEY'=6 S OUT="-3,Invalid key specified ("_KEY_")" Q
 ;
 L +^MAG(2006.587,0):1999999 ; Background process MUST wait
 S (N,D0)=0 F  S D0=$O(^MAG(2006.587,D0)) Q:'D0  D
 . S X=$G(^MAG(2006.587,D0,0)),ID=$P(X,"^",KEY) Q:ID'=OLD
 . S N=N+1
 . S LOC=$P(X,"^",7),SVC=$P(X,"^",1)
