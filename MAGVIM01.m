MAGVIM01 ;WOIFO/DAC/NST/BT,ISI/SAF,NST - Utilities for RPC calls for DICOM file processing ; 23 May 2014 14:10 PM
 ;;3.0;IMAGING;**118,local**;Mar 19, 2002;Build 4525;May 01, 2013
...
UPDITEM(OUT,ID,EXPSTAT,NEWSTAT,MESSAGE,UPDUSR,UPDAPP) ; Update work item status and create an entry in the work history file
 ...
 L +^MAGV(2006.941,ID):1999999  ;longest number supported by GT.M - ISI/SAF
...
GETNEXT(OUT,ETYPE,EXPSTAT,NEWSTAT,UPDUSR,UPDAPP,LOCATION) ; Find last update work item on worklist type provided
 N SSEP,FDA,ID,TYPE,STATUS,MATCH,APPIEN,LOCATIN2,LOCIEN,ETYPEIEN,ESTATIEN,ELOCIEN
 K OUT
 S SSEP=$$STATSEP
 ...
 S ID="",MATCH=0
 F  S ID=$O(^MAGV(2006.941,"C",ETYPEIEN,ESTATIEN,ELOCIEN,ID)) Q:ID=""  D  Q:MATCH=1
 . L +^MAGV(2006.941,ID):1999999  ;longest number supported by GT.M - ISI/SAF 
 ...
 . I EXPSTAT=STATUS,ETYPE=TYPE,LOCATION=LOCATIN2 S MATCH=1 Q
 . L -^MAGV(2006.941,ID)
 . Q
