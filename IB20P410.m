IB20P410	;ISP/PHH - POST-INIT FOR IB*2.0*410; 11/25/2008
	;;2.0;INTEGRATED BILLING;**410**;21-MAR-94;Build 1
	;
PRE	; set up check points for pre/post-init
	N %
	S %=$$NEWCP^XPDUTL("THRESH","THRESH^IB20P410")
	S %=$$NEWCP^XPDUTL("MCRDED","MCRDED^IB20P410")
	S %=$$NEWCP^XPDUTL("PRIOR","PRIOR^IB20P410")
	Q
	;
THRESH	; Pension Threshold
	N IBA,IBERRM,IBRN,IBTYPE,IBX,DA,DIK
	S IBTYPE="Pension Threshold"
	D BMES^XPDUTL("Filing CY 2009 Pension Threshold rates.")
	S IBX=3071201
	F  S IBX=$O(^IBE(354.3,"B",IBX)) Q:'IBX  D  ; remove any records since 12/01/2007
	. S IBRN=0
	. F  S IBRN=$O(^IBE(354.3,"B",IBX,IBRN)) Q:'IBRN  D
	.. S DIK="^IBE(354.3,",DA=IBRN D ^DIK
	S IBA(354.3,"+1,",.01)=3081201 ; effective date for CY 2009 values
	...
	D UPDATE^DIE("","IBA","","IBERRM") ; file the new record for CY 2009
	I $D(IBERRM) D
...
PRIOR	;This code sets up the variables and calls the routine to print or print-and-update the
	...
	;
	Q:'$D(^IBA(354.1,"APRIOR",3071201))  ; quit if the "APRIOR" x-ref is not set for 12/1/07.
	...
	S IBBMES=$S($D(ZTSK):"This job has been queued for NOW, as task number "_ZTSK_".",1:"This job could not be queued. Please edit the 12/1/08 threshold through the 'Add Income Thresholds' option, which allows you to queue this job.")
...
MMSG	; MailMan message to report update problem to billing groups, patch installer and patch developer
	...
	S IBPARAM("FROM")="PATCH IB*2.0*410 CY 2009 RATE UPDATE"
	F IBGROUP="IB EDI SUPERVISOR","IB ERROR","MCCR" D
	. I $D(^XMB(3.8,"B",IBGROUP)) S IBGROUP="G."_IBGROUP,XMY(IBGROUP)=""
	S XMY(DUZ)=""
	;
	S IBC=0
	S IBC=IBC+1,IBTXT(IBC)="This message has been sent by patch IB*2.0*410. If you have received this"
	S IBC=IBC+1,IBTXT(IBC)="message, it indicates that the patch encountered some difficulty in filing"
	S IBC=IBC+1,IBTXT(IBC)="the CY 2009 "_IBTYPE_" rates as outlined in the patch description."
	...
	S IBC=IBC+1,IBTXT(IBC)="You can consult the IB*2.0*410 patch description for additional information."
