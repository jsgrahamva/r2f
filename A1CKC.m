A1CKC ; GENERATED FROM 'A1CK VARO/DHCP' INPUT TEMPLATE(#1505), FILE 2;08/22/16
 D DE G BEGIN
...
C6F1 N X,X1,X2 S DIXR=799 D C6X1(U) K X2 M X2=X D C6X1("O") K X1 M X1=X
 D
...
C15S S X="" G:DG(DQ)=X C15F1 K DB
 D ^A1CKC6
C15F1 N X,X1,X2 S DIXR=799 D C15X1(U) K X2 M X2=X D C15X1("O") K X1 M X1=X
 D
 . D FC^DGFCPROT(.DA,2,.361,"KILL",$H,$G(DUZ),.X,.X1,.X2,$G(XQY0)) Q
 K X M X=X2 D
 . D FC^DGFCPROT(.DA,2,.361,"SET",$H,$G(DUZ),.X,.X1,.X2,$G(XQY0)) Q
...
X15 S DFN=DA D EV^DGLOCK I $D(X) D ECD^DGLOCK1
 Q
 ;
16 D:$D(DG)>9 F^DIE17,DE S DQ=16,DW="TYPE;1",DV="P391'a",DU="",DLB="TYPE",DIFLD=391
...
C16S S X="" G:DG(DQ)=X C16F1 K DB
 D ^A1CKC8
C16F1 N X,X1,X2 S DIXR=664 D C16X1(U) K X2 M X2=X D C16X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . K ^DPT("APTYPE",X,DA)
 K X M X=X2 I $G(X(1))]"" D
 . S ^DPT("APTYPE",X,DA)=""
