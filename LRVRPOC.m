LRVRPOC	;DALOI/JMC - POINT OF CARE VERIFICATION ;01/27/15  21:36
	;;5.2;LAB SERVICE;**290,350,250005,250068,WVEHR,LOCAL**;Sep 27, 1994;Build 6
	;
...
LOOK	; Check for data
	...
	;S DFN=$$GETDFN^LRVRPOCU(LASSN) ;WV
	; WV/JMC 12 Sept 2010 - added code in following section to support mutiple type of identifiers.
	;                       Supports SSN, ICN, PI (patient identifer) and MR ( medical record number).
	;                       See LA7VHU2 for further info on identifier types
	N DFN,I,LRPI S DFN=""
	F I="ICN","MR","PI" S LRPI(I)=$P($G(^LAH(LRLL,1,LAIEN,.1,"PID",I)),"^") ;WV
	;
	; If id is SSN ;WV
	I LASSN'="" S DFN=$$GETDFN^LA7VHLU2(LASSN,1) ;WV
	;
	; If no SSN then try ICN (VA Integration Control Number) ;WV
	I DFN<1,LRPI("ICN")'="" S DFN=$$GETDFN^LA7VHLU2(LRPI("ICN"),2) ;WV
	;
	; If no SSN, ICN then try PI (Patient Internal Identifier) ;WV
	I DFN<1,LRPI("PI")'="" S DFN=$$GETDFN^LA7VHLU2(LRPI("PI"),3) ;WV
	;
	; If no SSN, ICN, PI then try MR (Medical Record Number) ;WV
	I DFN<1,LRPI("MR")'="" S DFN=$$GETDFN^LA7VHLU2(LRPI("MR"),4) ;WV
	; End WorldVistA Change JMC
	;
	I 'DFN D  Q
