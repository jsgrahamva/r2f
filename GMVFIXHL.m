GMVFIXHL	       ;HIOFO/FT - HOSPITAL LOCATION FIX ; 9/20/12 3:55pm
	       ;;5.0;GEN. MED. REC. - VITALS;**24,LOCAL**;Oct 31, 2002;Build 10
	       ;
	       ...
	       ;
	       ; VEN/SMH : local mods (2/2) to fix a call to KIDS which Cache illegally allows to execute.
	       ;
EN	     ;
...
QUEUE	  ; The problem started when GMRV*5.0*3 was installed.
	       ...
	       ;;BEGIN LOCAL MOD
	       ; Local Mod - VEN SMH 1/2
	       ; Was; D INSTALDT^XPDUTL("GMRV*5.0*3",.GMVDATES) ;VEN/SMH - b4
	       N % S %=$$INSTALDT^XPDUTL("GMRV*5.0*3",.GMVDATES) ; VEN/SMH - after
	       ;;END LOCAL MOD
	       ...
	       K GMVDATES
	       ;;BEGIN LOCAL MOD
	       ; Local Mod - VEN SMH 2/2
	       ; D INSTALDT^XPDUTL("GMRV*5.0*22",.GMVDATES) ;VEN/SMH - b4
	       N % S %=$$INSTALDT^XPDUTL("GMRV*5.0*22",.GMVDATES) ; VEN/SMH - After
	       ;;END LOCAL MOD
	       I $G(GMVDATES(0))=0 D  Q
	       .K GMVTEXT
