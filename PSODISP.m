PSODISP	;BIR/SAB,PWC - MANUAL BARCODE RELEASE FUNCTION ;8:31 AM  2 Aug 2015
	;;7.0;OUTPATIENT PHARMACY;**15,71,131,156,185,148,247,200,385,391,WVEHR,LOCAL**;DEC 1997;Build 2
	;
...
AC	K CX,PSODA,PSODT,PSRH,DA,DR,DIE,X,X1,X2,Y,RXP,CX,PX,REC,DIR,YDT,REC,RDUZ,DIRUT,PSOCPN,PSOCPRX,YY,QDRUG,QTY,TYPE,XTYPE,DUOUT,PSOPID
	...
	;WAS;AC1     W !! S PSIN=+$P($G(^PS(59.7,1,49.99)),"^",2)
	;WAS;S DIC("S")="I $D(^XUSEC(""PSORPH"",+Y))",DIC("A")="Enter PHARMACIST: ",DIC="^VA(200,",DIC(0)="QEAM" D ^DIC G:"^"[X EXIT K DIC G:$D(DTOUT)!($D(DUOUT))!($D(DIRUT))!(Y=-1) EXIT S PSRH=+Y
AC1	I $G(PSOAFYN)'="Y" W !! S PSIN=+$P($G(^PS(59.7,1,49.99)),"^",2)
	I $G(PSOAFYN)="Y" S PSIN=+$P($G(^PS(59.7,1,49.99)),"^",2)
	I $G(PSOAFYN)'="Y" S DIC("S")="I $D(^XUSEC(""PSORPH"",+Y))",DIC("A")="Enter PHARMACIST: ",DIC="^VA(200,",DIC(0)="QEAM" D ^DIC G:"^"[X EXIT K DIC G:$D(DTOUT)!($D(DUOUT))!($D(DIRUT))!(Y=-1) EXIT S PSRH=+Y
	I $G(PSOAFYN)="Y" S PSRH=DUZ,PSZAR="0"
...
BC	;
	...
	;Begin WorldVistA change
	;WAS;Q:$G(POERR)  W !! K CMOP,ISUF,DIR,LBL,LBLP S DIR("A")="Enter/Wand PRESCRIPTION number",DIR("?")="^D HELP^PSODISP",DIR(0)="FO" D ^DIR
	I $G(PSOAFYN)'="Y" Q:$G(POERR)  W !! K CMOP,ISUF,DIR,LBL,LBLP S DIR("A")="Enter/Wand PRESCRIPTION number",DIR("?")="^D HELP^PSODISP",DIR(0)="FO" D ^DIR
	I $G(PSOAFYN)="Y" S X=RXN,PSZAR="1"
	;End WorldVistA change
...
UPDATE	I $G(ISUF) W $C(7),!!?7,"Prescription "_$P(^PSRX(RXP,0),"^")_" - Original Fill on Suspense !",!,$C(7) Q
	...
	;Begin WorldVistA change
	;WAS;W !?7,"Prescription Number "_$P(^PSRX(RXP,0),"^")_" Released"
	I $G(PSOAFYN)'="Y" W !?7,"Prescription Number "_$P(^PSRX(RXP,0),"^")_" Released"
	;End WorldVistA change
