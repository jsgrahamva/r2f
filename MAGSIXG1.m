MAGSIXG1 ;WOIFO/EdM/GEK/SEB/SG - LIST OF IMAGES RPCS ; 2/23/09 9:51am
 ;;3.0;IMAGING;**8,48,59,93**;Dec 02, 2009;Build 2
...
GETIMGS(MAGOUT,FLAGS,FROMDATE,TODATE,MAXNUM,MISCPRMS) ;RPC [MAG4 IMAGE LIST]
 ...
 K ^TMP("MAGSIXG3",$J)
 ;
 I $L($T(DEBUG^ISIE1)) D DEBUG^ISIE1 ; In for Debugging. ; TESTING
 ;=== Validate parameters
 S ERROR=0
 D
 . N MISCDEFS
 ...
 . Q
 ;
 ;+++ ISI ECG Window List the Group entries.
 I $L($T(ISIP1^ISIE1)) D ISIP1^ISIE1(.MAGDATA,.MISCPRMS)
 ;+++
 ;=== Cleanup
 K ^TMP("MAGSIXG3",$J)
