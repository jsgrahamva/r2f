MAGDIR8A ;WOIFO/PMK,ISI/BT - Read a DICOM image file ; 01/12/2016  17:02
 ;;3.0;IMAGING;**11,51,local**;26-August-2005;Build 2
 ;; +---------------------------------------------------------------+
...
PIDCHECK() ; compare VistA patient ID with DICOM patient ID
 ...
 ;
 ; lookup patient in VistA database **ISI/BT local mod**
 ...
 ;
 ; ** ISI/BT local mod - PID may in VA("PID") or VA("BID")
 ;I $$SITE^VASITE()["OROVILLE" S (IDVAH,DCMPID)=VA("PID")  ; Need HRN for Oroville
 ;E  S IDVAH=$P(VADM(2),"^"),DCMPID=$P(VADM(2),"^",2)
 ;
 S IDVAH=$P(VADM(2),"^"),DCMPID=$P(VADM(2),"^",2)
 I +IDVAH'=+IDDCM,+VA("PID")=+IDDCM!(+VA("BID")=+IDDCM) S (IDVAH,DCMPID)=IDDCM
 ; ** end local mod **
 ;
 ...
 ;
 ;updated to support English Name field in Jordan - saf ISI 9/24/11
 ;need to add additional check for DOB at least
 ...
 S CHECK=CHECK+(5*(+IDDCM=+IDVAH)) ; patient id requires an exact match - SAF strip leading 0s 6/27/12
