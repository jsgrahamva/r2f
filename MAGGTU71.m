MAGGTU71 ;WOIFO/GEK - Silent calls for Queing functions from GUI, cont ;  [ 06/20/2001 08:57 ]
 ;;3.0;IMAGING;**46,59**;Nov 27, 2007;Build 2
...
ABSJB(MAGRY,DATA) ;RPC [MAG ABSJB] SET ABSTRACT AND/OR JUKEBOX QUEUES
 ...
 ;             = "0^error message"
 ;  
 S ^TMP("MAGGTU71","ABSJB","DATA")=$G(DATA)           
 ...
 ; ISI - test to see if we want to copy to JB by BP
 I MAGIENJB D  ; ISI
 . S DAPLC=$$DA2PLC^MAGBAPIP(MAGIENJB,"F")
 . S ISIBKUP=+$P($G(^MAG(2006.1,DAPLC,"ISI")),"^",1)
 . ; HERE, we Stopped the JB Copy by BP because
 . ; of the setting of the IMAGE BACKUP OPTIONS field.
 . ; if ISIBKUP = 0 then we want the Queue JBCopy, other wise we don't.
 . I ISIBKUP S MAGIENJB=0 
 . Q
 ...
 I MAGIENJB S QMSG=$S(QMSG="":"Setting JukeBox Queue",1:QMSG_" and JukeBox Queue")
