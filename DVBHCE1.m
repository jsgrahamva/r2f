...
X30 S DVBSCR=1 D ^DVBHS5 S DVBJ2=1
 Q
31 S DW="TYPE;1",DV="P391'a",DU="",DLB="TYPE",DIFLD=391
...
C31F1 N X,X1,X2 S DIXR=664 D C31X1(U) K X2 M X2=X D C31X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . K ^DPT("APTYPE",X,DA)
...
C31F2 Q
X31 Q
32 D:$D(DG)>9 F^DIE17,DE S DQ=32,DW="VET;1",DV="SXa",DU="",DLB="VETERAN (Y/N)?",DIFLD=1901
 S DE(DW)="C32^DVBHCE1"
 S DU="Y:YES;N:NO;"
 G RE
...
X32 I $D(X) S:'$D(DPTX) DFN=DA D:'$D(^XUSEC("DG ELIGIBILITY",DUZ)) VAGE^DGLOCK:X="Y" I $D(X) D:$D(DFN) EV^DGLOCK
 Q
 ;
33 D:$D(DG)>9 F^DIE17,DE S DQ=33,DW=".3;1",DV="SXa",DU="",DLB="SERVICE CONNECTED?",DIFLD=.301
 S DE(DW)="C33^DVBHCE1"
 S DU="Y:YES;N:NO;"
 G RE
...
C36S S X="" G:DG(DQ)=X C36F1 K DB
 D ^DVBHCE7
C36F1 N X,X1,X2 S DIXR=799 D C36X1(U) K X2 M X2=X D C36X1("O") K X1 M X1=X
 D
