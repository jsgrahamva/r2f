MAGDQR03 ;WOIFO/EdM - Imaging RPCs for Query/Retrieve ; 17 Feb 2010 11:18 AM
 ;;3.0;IMAGING;**51,54,66**;Mar 19, 2002;Build 1;Sep 02, 2010
...
Q0080018 ;U Image Instance UID
 ...
 ;
 ; *** MLS/ISI mod. start ***
Q0200013 ;R Instance Number (0020,0013)
 S V(T)=""
 I MAGIEN="" Q
 N PARENT,OUT,Z
 S PARENT=$P($G(^MAG(2005,MAGIEN,0)),"^",10)
 S:'PARENT PARENT=MAGIEN
 I $D(^MAG(2005,PARENT,1,"ADCM")) D  
 . S Z=0,OUT=0 F  S Z=$O(^MAG(2005,PARENT,1,Z)) Q:('Z!(OUT))  D  
 . . Q:$P($G(^MAG(2005,PARENT,1,Z,0)),U)'=MAGIEN
 . . S OUT=1
 . . S V(T)=$P($G(^MAG(2005,PARENT,1,Z,0)),U,3)
 Q
 ; *** MLS/ISI mod. end ***
 ;
...
Q0200011 ;R Series number
 ...
 . . Q:$P($G(^MAG(2005,PARENT,1,Z,0)),U)'=MAGIEN
 . . S OUT=1
 . . S V(T)=$P($G(^MAG(2005,PARENT,1,Z,0)),U,2)
 Q
 ; *** MLS/ISI mod. end ***
 ;
