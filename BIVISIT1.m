BIVISIT1 ;IHS/CMI/MWR - CREATE/EDIT VISITS.; MAY 10, 2010
 ;;8.5;IMMUNIZATION;**5**;JUL 01,2013;Build 9
...
VISIT(BIDFN,BIDATE,BICAT,BILOC,BIOLOC,BISITE,BIVSIT,BIERR,BINOM) ;EP
 ...
 ;------> RPMS (Institution)
 I $$RPMS^BIUTL9(),$P(^AUPNVSIT(+BIVSIT,0),U,6)'=BILOC D  Q
 ...
 ;
 ;------> VISTA (Hospital Location)
 I '$$RPMS^BIUTL9(),$P(^AUPNVSIT(+BIVSIT,0),U,22)'=BILOC D  Q
 .N BIFLD S BIFLD(.22)=BILOC,BIFLD(2101)=BIOLOC
 .D FDIE^BIFMAN(9000010,BIVSIT,.BIFLD,.BIERR)
 .I $G(BIERR) D ERRCD^BIUTL2(433,.BIERR) S BIERR="1^"_BIERR
 ;
