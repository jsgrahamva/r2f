PSOORFIN ;BIR/SAB-finish cprs orders ;3:46 AM  23 Aug 2015
 ;;7.0;OUTPATIENT PHARMACY;**7,15,27,32,44,46,84,106,111,117,131,146,139,195,225,315,266,338,391,372,416,WVEHR,LOCAL**;DEC 1997;Build 1
 ;
 ...
 ;WAS;D:'$D(PSOPAR) ^PSOLSET I '$D(PSOPAR) D MSG^PSODPT G EX
...
C D KQ F  S ORD=$O(^PS(52.41,"AC",PAT,"C",ORD)) Q:'ORD!($G(POERR("QFLG")))  I $P(^PS(52.41,ORD,0),"^",3)'="DC",$P(^(0),"^",3)'="DE" D LK1,ORD S CLI=1
 ...
 Q
 ;Begin WorldVistA change - reviewed by Sam Habiel 2018/09/03
PAT ;WAS;W ! K MEDP,MEDA,POERR("DFLG"),DIR D KQ S PSOSORT="PATIENT"
 ;WAS;S DIR("?")="^D PT^PSOORFI1",DIR("A")="All Patients or SinglePatient",DIR(0)="SBM^A:ALL;S:SINGLE;E:EXIT",DIR("B")="SINGLE"
 ;WASD ^DIR K DIR G:$D(DIRUT)!(Y="E") EX I Y="S" S PSOSORT=PSOSORT_"^"_"SINGLE" G SPAT
...
SPAT K MEDA,MEDP,PSOQFLG,PSORX("FN"),BINGCRT,BINGRTE D KQ,KV^PSOVER1
 ;Begin WorldVistA change
 ;WAS;S DIR(0)="FO^2:30",DIR("A")="Select Patient",DIR("?")="^D HELP^PSOORFI2" D ^DIR I $E(X)="?" G SPAT
 ;WAS;G:$D(DIRUT) EX D KV^PSOVER1
 ;WAS;S DIC(0)="EQM",DIC=2,DIC("S")="I $D(^PS(52.41,""AOR"",+Y,PSOPINST))"
 I $G(PSOAFDON)=1 G EX
 ...
 .S X=PAT D ULP
 ;Begin WorldVistA change
 ;WAS;S ORD=0 F  S ORD=$O(^PS(52.41,"P",PAT,ORD)) Q:'ORD!($G(POERR("QFLG")))  D:'$P($G(^PS(52.41,ORD,0)),"^",23)
