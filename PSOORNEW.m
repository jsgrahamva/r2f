PSOORNEW	;BIR/SAB - display orders from oerr ;5:46 AM  8 Sep 2015
	;;7.0;OUTPATIENT PHARMACY;**11,23,27,32,55,46,71,90,94,106,131,133,143,237,222,258,206,225,251,386,390,391,372,416,431,313,408,436,WVEHR,LOCAL**;DEC 1997;Build 1
	;
...
ACP	;
	...
	;WAS;D:'$G(PSORX("DFLG")) DOSCK^PSODOSUT("N") I $G(PSORX("DFLG")) G DSPL
	D:('$G(PSORX("DFLG"))&($G(PSOAFYN)'="Y")) DOSCK^PSODOSUT("N") I $G(PSORX("DFLG")) D CLEAN^PSOVER1 G DSPL
	...
	;Begin WorldVistA change
	;WAS;D STOP^PSONEW2,DISPLAY^PSONEW2,^PSONEWF
	...
	;
	;Begin WorldVistA change
	;WAS;K PSOCPZ("DFLG") D KV S DIR(0)="Y",DIR("A")="Are you sure you want to Accept this Order",DIR("B")="NO" D ^DIR I $D(DIRUT) D KV K DRET,PSOANSQ,PSOANSQD S VALMBCK="Q" Q
	...
	D EOJ^PSONEW
	;Begin WorldVistA change
	;WAS;ABORT   S VALMBCK="Q",DIR(0)="E",DIR("?")="Press Return to continue",DIR("A")="Press Return to Continue" D ^DIR,CLEAN^PSOVER1,KV
ABORT	I $G(PSOAFYN)'="Y" S VALMBCK="Q",DIR(0)="E",DIR("A")="Press Return to Continue" D ^DIR,CLEAN^PSOVER1,KV
	I $G(PSOAFYN)="Y" D CLEAN^PSOVER1,KV
	;End WorldVistA change
	Q
...
DRGMSG	;
	;Begin WorldVistA change
	;WAS;F SG=1:1:$L($P(^PSDRUG(PSODRUG("IEN"),0),"^",10)) S:$L(^TMP("PSOPO",$J,IEN,0)_" "_$P($P(^PSDRUG(PSODRUG("IEN"),0),"^",10)," ",SG))>80 IEN=IEN+1,$P(^TMP("PSOPO",$J,IEN,0)," ",20)=" " D
	;WAS;.S:$P($P(^PSDRUG(PSODRUG("IEN"),0),"^",10)," ",SG)'="" ^TMP("PSOPO",$J,IEN,0)=$G(^TMP("PSOPO",$J,IEN,0))_" "_$P($P(^PSDRUG(PSODRUG("IEN"),0),"^",10)," ",SG)
	;WAS;K SG
	D DRGMSG^PSOORNW2 Q
	;End WorldVistA change
	Q
