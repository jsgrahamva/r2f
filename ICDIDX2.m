ICDIDX2	;DLS/DEK - MUMPS Cross Reference Routine for History ;04/21/2014
	;;18.0;DRG Grouper;**57,WVEHR,LOCAL**;Oct 20, 2000;Build 3
...
SNUM(ICD)	; Code            .01      AN1    Set
	...
	Q:'$L($G(X))  Q:+($G(DA))'>0
	;BEGIN WorldVistA Change ; 10/2015 DJW
	;Treat NUM as numeric identifier (strlit) not an int
	;WAS; S NUM=$$NUM^ICDEX(X) Q:+NUM'>0
	;WAS; S @(RT_""""_"AN"_+SYS_""","_+NUM_","_+DA_")")=""
	S NUM=$$NUM^ICDEX(X) Q:NUM=""
	S @(RT_""""_"AN"_+SYS_""","""_NUM_""","_+DA_")")=""
...
KNUM(ICD)	; Code            .01      AN1    Kill
	...
	;BEGIN WorldVistA Change ; 10/2015 DJW
	;Treat NUM as numeric identifier (strlit) not an int
	;WAS; Q:'$L($G(X))  Q:+($G(DA))'>0  S NUM=$$NUM^ICDEX(X) Q:+NUM'>0
	;WAS; K @(RT_""""_"AN"_+SYS_""","_+NUM_","_+DA_")")
	Q:'$L($G(X))  Q:+($G(DA))'>0  S NUM=$$NUM^ICDEX(X) Q:NUM=""
	K @(RT_""""_"AN"_+SYS_""","""_NUM_""","_+DA_")")
	;END WorldVistA Change
	Q
SNUM2(ICD)	; Coding System   1.1      AN2    Set
	...
	;BEGIN WorldVistA Change ; 10/2015 DJW
	;Treat NUM as numeric identifier (strlit) not an int
	;WAS; S NUM=$$NUM^ICDEX(COD) Q:+NUM'>0
	;WAS; S @(RT_""""_"AN"_+SYS_""","_+NUM_","_+DA_")")=""
	S NUM=$$NUM^ICDEX(COD) Q:NUM=""
	S @(RT_""""_"AN"_+SYS_""","""_NUM_""","_+DA_")")=""
	;END WorldVistA Change
	Q
KNUM2(ICD)	; Coding System   1.1      AN2    Kill
	...
	;BEGIN WorldVistA Change ; 10/2015 DJW
	;Treat NUM as numeric identifier (strlit) not an int
	;WAS; Q:'$L(COD)  S NUM=$$NUM^ICDEX(COD) Q:+NUM'>0
	;WAS; K @(RT_""""_"AN"_+SYS_""","_+NUM_","_+DA_")")
	Q:'$L(COD)  S NUM=$$NUM^ICDEX(COD) Q:NUM=""
	K @(RT_""""_"AN"_+SYS_""","""_NUM_""","_+DA_")")
	;END WorldVistA Change
	Q
