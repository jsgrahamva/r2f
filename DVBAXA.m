DVBAXA ; GENERATED FROM 'DVBA C ADD 2507 PAT' INPUT TEMPLATE(#965), FILE 2;08/22/16
 D DE G BEGIN
...
C5F1 N X,X1,X2 S DIXR=795 D C5X1(U) K X2 M X2=X D C5X1("O") K X1 M X1=X
 D
...
X7 S DFN=DA D EV^DGLOCK I $D(X) S L=$S($D(^DPT(DA,0)):$P(^(0),U,9),1:X) W:X?1"SS".E "  ",L S:X?1"SS".E X=L K:$L(X)>9 X Q:'$D(X)  I X'=L K:$L(X)>8!($L(X)<7)!'(X?.N) X
 ...
 Q
 ;
8 D:$D(DG)>9 F^DIE17,DE S DQ=8,DW=".31;4",DV="*P4'XR",DU="",DLB="CLAIM FOLDER LOCATION",DIFLD=.314
 S DE(DW)="C8^DVBAXA"
 S DU="DIC(4,"
 G RE
...
X8 S DIC("S")="I $$CFLTF^DGREGDD(Y)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW=".3;1",DV="SXa",DU="",DLB="SERVICE CONNECTED?",DIFLD=.301
 S DE(DW)="C9^DVBAXA"
 S DU="Y:YES;N:NO;"
 G RE
...
X10 S DFN=DA D EV^DGLOCK Q:'$D(X)  K:+X'=X!(X>100)!(X<0)!(X?.E1"."1N.N) X I $D(X),$D(^DPT(DA,.3)),$P(^(.3),U,1)'="Y" W !?4,*7,"Only applies to service-connected applicants." K X
 Q
 ;
11 D:$D(DG)>9 F^DIE17,DE S DQ=11,DW="TYPE;1",DV="P391'a",DU="",DLB="TYPE",DIFLD=391
...
C11F2 Q
X11 Q
12 D:$D(DG)>9 F^DIE17,DE S DQ=12,DW="VET;1",DV="SXa",DU="",DLB="VETERAN (Y/N)?",DIFLD=1901
 S DE(DW)="C12^DVBAXA"
 S DU="Y:YES;N:NO;"
 G RE
...
X12 I $D(X) S:'$D(DPTX) DFN=DA D:'$D(^XUSEC("DG ELIGIBILITY",DUZ)) VAGE^DGLOCK:X="Y" I $D(X) D:$D(DFN) EV^DGLOCK
 Q
 ;
13 D:$D(DG)>9 F^DIE17,DE S DQ=13,DW=".36;1",DV="*P8'XaR",DU="",DLB="PRIMARY ELIGIBILITY CODE",DIFLD=.361
...
C13S S X="" G:DG(DQ)=X C13F1 K DB
 D ^DVBAXA4
C13F1 N X,X1,X2 S DIXR=799 D C13X1(U) K X2 M X2=X D C13X1("O") K X1 M X1=X
 D
...
X13 S DFN=DA D EV^DGLOCK I $D(X) D ECD^DGLOCK1
 Q
 ;
14 D:$D(DG)>9 F^DIE17,DE S DQ=14,DW=".52;5",DV="RSXR",DU="",DLB="POW STATUS INDICATED?",DIFLD=.525
...
C14S S X="" G:DG(DQ)=X C14F1 K DB
 D ^DVBAXA6
C14F1 N X,X1,X2 S DIXR=646 D C14X1(U) K X2 M X2=X D C14X1("O") K X1 M X1=X
 D
